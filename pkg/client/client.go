...
func (c *Client) SendMessage(channel *Channel, payload []byte, opt ...SendOption) (*wkproto.SendPacket, int64, error) {
	opts := NewSendOptions()
	if len(opt) > 0 {
		for _, op := range opt {
			_ = op(opts)
	...
	dataSize, err := c.appendPacket(packet)
	if err != nil {
		return nil, 0, err
	}
...
func (c *Client) appendPacket(packet wkproto.Frame) (int64, error) {
